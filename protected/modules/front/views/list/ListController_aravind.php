	...
	function actionEpooja()
	{
	  ...
	  $criteria->condition =  $condition;
		...
		if(Yii::app()->request->isAjaxRequest){
			...
			$this->render('poojaview_old', array(
				'dataProvider' => $dataProvider,
				'dataProvider1' => $dataProvider1,
				'page_size'=>'12',
                'items_count'=>$count,
...
//		 
//		 if(Yii::app()->request->isAjaxRequest){
//				$this->renderPartial('iyerview_aravind', array(
//				'dataProvider' => $dataProvider,
//				'dataProvider1' => $dataProvider1,
//				'pages' => $pages
//			));
	   ...
	   //get criteria
		$criteria = new CDbCriteria();
	 	$condition  = ' role = 4 AND registration_completed=1 ';
                ...
                if(isset($_REQUEST['title'])){
		 $condition .= ' AND first_name  LIKE  "%'.$_REQUEST['title'].'%"';
		}

                if(isset($_REQUEST['categories'])){
                    $criteria->join = " JOIN iyercategories ic ON ic.id=ug.iyer_id ";
		 $condition .= ' AND ic.category  LIKE  "%'.$_REQUEST['categories'].'%"';
		}
                if(isset($_REQUEST['experience'])){
                   
                    
		 $condition .= ' AND ug.iyer_experience  IN ('.implode(',',$_REQUEST['experience']).')';
		}
		...
		//get count
		$count = User::model()->count($criteria1);
		$result1 = User::model()->findAll($criteria1);
	...
	$this->pageTitle =' Temples Unlimited | News & Events';
	   ...
	   $condition1 .= ' `event_end_date` >= "'.date('Y-m-d').'" ';
	     ...
	     if(isset($_REQUEST['events_value']))
		{
		 ...
		 if(Yii::app()->session['events_value']=='today')
		 {
		   $condition1 .= ' and `event_start_date` <= "'.date('Y-m-d').'"  and   `event_start_date` <= "'.date('Y-m-d').'" ';
		 }
		 else if(Yii::app()->session['events_value']=='tomorrow')
		 {
		  $tomorrow = date("Y-m-d", time()+86400); 
		  $condition1 .= ' and `event_start_date` <= "'.$tomorrow.'"  AND "'.$tomorrow.'" <= `event_end_date` ';
		 }
		else if(Yii::app()->session['events_value']=='week')
		{
				$dt =  strtotime(date('Y-m-d'));
			    ...
			    $condition1 .= ' and `event_start_date` >="'.$start.'"  and  `event_start_date`  <= "'.$end.'"';
		}
		else if(Yii::app()->session['events_value']=='weekend')
		{
				$dt =  strtotime(date('Y-m-d'));
				 ...
				 $end  = date('Y-m-d', strtotime('this sunday', $dt));
			 	  ...
			 	  $condition1 .= ' and `event_end_date` >= "'.$start.'"  and   `event_end_date` <= "'.$end.'" ';
		}
		else if(Yii::app()->session['events_value']=='month')
		{
		    $dt = strtotime(date('Y-m-d'));
			$start = date('Y-m-d', strtotime('first day of this month', $dt));
			$end = date('Y-m-d', strtotime('last day of this month', $dt));
			$condition1 .= ' and `event_start_date` >="'.$start.'"  and  `event_end_date`  <= "'.$end.'"';
		}
		else if(Yii::app()->session['events_value']=='all')
		{
		  $condition1 .='';
		...
		{	
        $this->render('events1',array('dataProvider'=>$dataProvider1,'pages'=>$pages,'condition'=>$condition1));
		}
	}
