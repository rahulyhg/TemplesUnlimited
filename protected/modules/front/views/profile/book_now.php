...
<div class="wrapper" style="margin-left:15%; margin-bottom:10%;" id="testdiv" >
...
<br />
	...
	<button type="button" class="sc-button alignleft profile-font"  style="background-color: #CB202D; color: #fff;font-style:bold; font-size:13px;border-radius:5px !important; border:2px solid #CB202D;" onclick="return updateaddress();" >Save</button>
	
	<button type="button" class="sc-button alignleft profile-font "  onclick="$('#address_div').hide();"  style="background-color: #828282; color: #fff;font-style:bold; font-size:13px;border-radius:5px !important; border:2px solid #828282;"  >Cancel</button>
...
<p><strong>Your Order Details - <?php  echo '1'; ?> item(s)</strong></p>
...
<form novalidate="novalidate" class="wpcf7-form" method="post" id="cartpage" action="<?php echo Yii::app()->createUrl('front/profile/placeorderlast'); ?>">
	...
	<div class="left" style="width: 120px;">Qty :<input style="padding:1%; width:20%;margin-top: -22px;" onkeypress="return isNumberKey(event)"  onkeyup="chage_amount(this.value,<?php echo $cart['product_id'];  ?>,<?php echo $cart['amount']; ?>,'<?php echo $cart['shipping_amount'] ?>')" class="cart-quant" value="1" id="quantity_<?php echo $cart['product_id'];  ?>" type="text" name="quantity_<?php echo $cart['product_id'];  ?>">
	...
	<div class="left" style="width:200px">Shipping Charges :</div><div class="right"><strong><?php echo ($cart['shipping_amount']!='0')?helpers::to_currency($cart['shipping_amount']):'Free'; ?></strong></div>	
		...
		<input type="hidden" name="variations" id="variations" value="<?php echo $cart['variations']; ?>"/>
		<input type="hidden" name="product_id" id="product_id" value="<?php echo  $cart['product_id'];  ?>"/>
		<input type="hidden" name="type" id="type" value="<?php echo "product";  ?>"/>
		
			<input type="hidden" name="total_amount_cart" id="total_amount_cart" value="<?php echo  $cart['amount'] + $cart['shipping_amount'];  ?>" />
	...
	<div class="left" style="width: 120px;">Qty :<input style="padding:1%; width:20%;margin-top: -22px;" onkeypress="return isNumberKey(event)"  onkeyup="chage_amount(this.value,<?php echo $cart['product_id'];  ?>,<?php echo $cart['amount']; ?>,'<?php echo $cart['shipping_amount'] ?>')" class="cart-quant" value="1" id="quantity_<?php echo $cart['product_id'];  ?>" type="text" name="quantity_<?php echo $cart['product_id'];  ?>">
			...
			</div><div class="right"><p class="right"><strong><span id="amount_<?php echo $cart['product_id'];  ?>"><?php echo helpers::to_currency($cart['amount']); ?></span></strong></p></div>
	<div class="left" style="width:200px">Shipping Charges :</div><div class="right"><strong><?php echo ($cart['shipping_amount']!='0')?helpers::to_currency($cart['shipping_amount']):'Free'; ?></strong></div>	
	</div>
	   ...
	   <input type="hidden" name="amount_shipping_values_<?php echo $cart['product_id'];  ?>" id="amount_shipping_values_<?php echo $cart['product_id'];  ?>" value="<?php echo  $cart['shipping_amount']; ?>" />
		...
		<input type="hidden" name="variations" id="variations" value="<?php echo $cart['variations']; ?>"/>
		<input type="hidden" name="product_id" id="product_id" value="<?php echo  $cart['product_id'];  ?>"/>
		<input type="hidden" name="type" id="type" value="<?php echo "pooja";  ?>"/>
	...
	<?php $ids_string .=$cart['product_id'].',';  
				...
				<div class="sc-column sc-column-last one-third-last right cart-price">
					...
					<div class="right" id="total_book"><strong>&nbsp;<?php echo helpers::to_currency($sum_amount.".00"); ?></strong></div>
					</div>
...
<script>
function chage_amount(quantity,product_id,product_price,shipping_amount)
{
if(quantity =='0')
{
  quantity  ='1' ;
...
var grant = parseInt(total)+parseInt(shipping_amount); 

$('#total_book').html('<strong>Rs '+grant+'.00'+'</strong>');

$('#total_amount_cart').val(parseInt(total)+parseInt(shipping_amount));
}
}
