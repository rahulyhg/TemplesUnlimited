...
<div class="sc-column sc-column-last three-fourth-last" >
        <div class="">
          ...
          <div class="rule"></div>
          <br>
          <h4 align="center"> Total purchase amount is <?php echo isset($profilecart)? helpers::to_currency($profilecart[0]->temp_total_amount):helpers::to_currency($total).".00"; ?></h4>
          <br>
...
unset(Yii::app()->session['cart1']);
unset(Yii::app()->session['cart2']);
}
else
{
$nonsite = Yii::app()->session['nonsite'];

$nonsiteuser =   Nonsiteuser::model()->find('name=:name',array(':name'=>$nonsite['name']));

$order = new Order;
$order->user_id =  $nonsiteuser->id;
$order->total_amount =   $total;
$order->user_type =  '1';
$order->user_name = $nonsiteuser->name;
$order->created = date('Y-m-d');
$order->status = "Accepted";
$order->save(false); 

Yii::app()->session['order'] = $order->id;
