    ...
    public function actionAuthenticate($provider='Facebook'){
        ...
        if(Yii::app()->hybridAuth->isAdapterUserConnected($provider)){
            ...
            if(isset($socialUser)){
				...
				/*$iyguide=User::model()->find('(email=:email or email=:email1) and role=:social_provider',array(':email'=>$socialUser->email,':email1'=>$socialUser->identifier."@twitter.com",':social_provider'=>$provider));
			  $userdeatils = User::model()->find('(email=:email or email=:email1) and social_provider=:social_provider',array(':email'=>$socialUser->email,':email1'=>$socialUser->identifier."@twitter.com",':social_provider'=>$provider));*/

$iyguide=User::model()->find('(email=:email or social_identifier=:social_identifier) and role=:social_provider',array(':email'=>$socialUser->email,':social_identifier'=>$socialUser->identifier,':social_provider'=>$provider));
			  $userdeatils = User::model()->find('(email=:email or social_identifier=:social_identifier) and social_provider=:social_provider',array(':email'=>$socialUser->email,':social_identifier'=>$socialUser->identifier,':social_provider'=>$provider));

			

			  if(count($userdeatils)   && !empty($userdeatils) && isset($userdeatils->user_id)){
			     $socialUser->exists = true;
			     $socialUser->email= $userdeatils->email;
			  }
			   ...
			   if(count($userdeatils)   && !empty($userdeatils) && isset($userdeatils->user_id)){
                   ...
                   if(isset($socialUser->email)){
                    $model->email = $socialUser->email;}
                     ...
                     /*$model->email = $socialUser->identifier."@twitter.com";*/
                    }
					$model->role = 2;
					$model->language = 1;
					$model->registration_completed = 1;
					$model->email_validated = 1;
				//	$model->gender = 'Mr';
                ...
                //    $model->social_info1 = hash('');
                 ...
                 //   ......
                    if($model->save()){
                       $user=$model; 
							$this->redirect(array('/front/user/socialregsuccess','id'=>$model->user_id));
                    }else{ 
