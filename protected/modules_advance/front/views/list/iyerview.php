...
<div id="maincontent">
<div class="wrapper">
   <h3 class="epooja"><?php if(isset($_REQUEST['cid']) && trim($_REQUEST['cid']) != ''){ ?>Related  <?php } ?>Iyers</h3>
 </div> 
...
<div class="sc-column one-half">
						...
						<input type="button" value="Search" class="dir-searchsubmit" style="margin-left:5px; width:20%; font-size:14px;" onclick="filterlist('<?php echo CHtml::normalizeUrl(array('/'.$this->getRoute().$urladditional)); ?>')">
						</span>
...
<!--<input type="checkbox" class="language-filters left" value="29" id="lang-filters-29" style="margin-right:12px;"><span>Not yet rated</span><span  id="categories-rating">(1)</span><br>-->
            ...
            <!--   <div class="ait-tab tab-content" data-ait-tab-title="Ratings">
                ...
                <div class="item-stars clearfix"><in if((int)$items->iyermoredetails->iyer_wh>=3 && (int)$items->iyermoredetails->iyer_wh<5)
				  $whoursvalues['3_5'] = $whoursvalues['3_5']+1;
				  else if((int)$items->iyermoredetails->iyer_wh>=5 && (int)$items->iyermoredetails->iyer_wh<7)
				  $whoursvalues['5_7'] = $whoursvalues['5_7']+1;
				  else if((int)$items->iyermoredetails->iyer_wh>=7 && (int)$items->iyermoredetails->iyer_wh<25)
 ...
 <div class="onecolumn">
	  ...
	  <div style="padding: 13px;" class="sc-column one-fourth">
	       ...
	       <?php foreach(  $whours as    $whourkey=>$whour){ ?>		 
			...
			<input type="checkbox" class="language-filters filteritem whours-filter" value="<?php echo  $whourkey; ?>" id="lang-filters-29" name="whours[]" <?php if(isset($_REQUEST['whours']) && in_array($whourkey,$_REQUEST['whours'])){ ?> checked="checked" <?php } ?> onchange="filterlist('<?php echo CHtml::normalizeUrl(array('/'.$this->getRoute().$urladditional)); ?>')" >
			<span class="flag flag-gb"></span>
...
function documentreadycontent(){
	$('.yiiPager li a').click(function(e){
	    e.preventDefault();
		filterlist('<?php echo CHtml::normalizeUrl(array('/'.$this->getRoute().$urladditional)); ?>/page/'+$(this).html());
