	 ...
	 <!--  <input type="button" class="btn btn-success pull-right" value="Add more activity plan" onclick="get_Guide_activities_form('<?php echo $id; ?>')" />	-->
	  <div class="activitieslists">
	   <?php  $iyercategories = explode(',',$iyer->iyer_categories); 
	               $poojas = Pooja::model()->getall_byids($iyercategories);
				  
				  if(count($poojas) && !empty($poojas)){
				     $count = 0;
				     foreach($poojas as $pooja){ $count++;
					    $model = new Iyeractivities;
						if(isset($_POST['Iyeractivities']) && isset($_POST['Iyeractivities'][$pooja->pooja_id]) )
						 $model->attributes = $_POST['Iyeractivities'][$pooja->pooja_id];
				        $this->renderPartial('_iyeractiviesform', array('model'=>$model,'pooja'=>$pooja,'key'=>$pooja->pooja_id,'id'=>$id, 'errors'=>$errors,)); 
					 }
				   }
